...
osSemaphoreId GreenEFHandle;
osSemaphoreId RedEFHandle;
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  /* definition and creation of GreenEF */
  osSemaphoreDef(GreenEF);
  GreenEFHandle = osSemaphoreCreate(osSemaphore(GreenEF), 1);


  /* definition and creation of RedEF */
  osSemaphoreDef(RedEF);
  RedEFHandle = osSemaphoreCreate(osSemaphore(RedEF), 1);


  /* USER CODE BEGIN RTOS_SEMAPHORES */
  /* add semaphores, ... */
  osSemaphoreWait(RedEFHandle, 1);//takes semaphore to make task wait
  osSemaphoreWait(GreenEFHandle, 1);//takes semaphore to make task wait
...
void StartGreenTask(void const * argument)
{
  ...
  for(;;)
  {
	  	...
	  	osSemaphoreWait(GreenEFHandle, portMAX_DELAY);//wait for EventFlag
...
void StartBlueTask(void const * argument)
{
  ...
  for(;;)
  {
		...
		osSemaphoreRelease(GreenEFHandle);//Send Event flag to Green

	    osDelay(6000);//delay 6 seconds


	    osSemaphoreRelease(RedEFHandle);//Send Event flag to Red
  }
...
void StartRedTask(void const * argument)
{
  ...
  for(;;)
  {
	  osSemaphoreWait(RedEFHandle, portMAX_DELAY);
